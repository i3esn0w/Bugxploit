...
class PluginManager(object):
    ...
    def _loadPlugin(self, plug):       
        ...
        if not loaded:
            self.addPlugin(plug)
            #security_info("loaded plugin %s"%plug.name)
            #print "%s: loaded plugin %s " % (system_info(), plug.name)
...
class DirectoryPluginManager(PluginManager):
    ...
    def __init__(self, directory,plugins=(), config={}):
        default_directory = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__),os.path.pardir)),"plugins\\"+directory)
